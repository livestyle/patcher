...
define(function(require, exports, module) {
	...
	function calculateDiff(ctx) {
		...
		// resolving because cached inital state will be replaced with
		// current source
		var prev = null;
		if ('previous' in data) {
			prev = data.previous;
	...
	function setInitialContent(ctx) {
		...
		livestyle.resolve(data.content || '', data, function(err, tree) {
			if (err) {
				return ctx.error('Unable to parse initial content: ' + err);
			}

			livestyle.cache.add(data.uri, tree, data.hash);

			// no need to respond anything, just tell that command is complete
			ctx.success();
		});
	}
